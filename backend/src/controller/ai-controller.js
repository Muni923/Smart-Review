...
const reviewCode = async (req, res) => {
    ...
    if (!language && !code) {
        return res.status(400).json({ review: "Please select language and enter code..." });
    }
    if (!code) {
        return res.status(400).json({ review: "Please paste your code..." });

    }
    if (!language) {
        return res.status(400).json({ review: "Please select language..." });
    }
    ...
    return res.status(200).json({ review: interaction.output_text });
};
