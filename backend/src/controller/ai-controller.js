...
const reviewCode = async (req, res) => {
    ...
    if (!language && !code) {
        res.status(400).json({ review: "Please select language and enter code..." });
    }
    if (!code) {
        res.status(400).json({ review: "Please paste your code..." });

    }
    if (!language) {
        res.status(400).json({ review: "Please select language..." });
    }
    ...
    const interaction = await ai.interactions.create({
     ...
     res.status(200).json({ review: interaction.output_text });
};
